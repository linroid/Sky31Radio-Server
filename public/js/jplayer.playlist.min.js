! function(t, s) {
	jPlayerPlaylist = function(s, i, e) {
		...
		this.current = 0, this.loop = !1, this.shuffled = !1, this.removing = !1, this.cssSelector = t.extend({}, this._cssSelector, s), this.options = t.extend(!0, {
			...
			t(this).jPlayer("play");
			/*控制文稿按钮更改onclick事件参数-start*/
			//console.log(l.playlist[l.current]);
			//alert(l.playlist[l.current]);
            wgid = l.playlist[l.current].id;
			bg = l.playlist[l.current].background;
			mp3 = l.playlist[l.current].mp3;
			//reqUyan(wgid);
			//reqChangYan(wgid);
			changeBg(bg);
			changeDownload(mp3);
			$('#showTop').attr('onclick','reqWG('+wgid+');')
			/*控制文稿按钮更改onclick事件参数-end*/
		...
		_createListItem: function(s) {
			...
			return e += "<a href='javascript:;' class='" + this.options.playlistOptions.itemClass + "' tabindex='1'>" +" <span class='jp-title' title="+s.title+"> "+ s.title + "</span>"+ (s.artist ? " <span class='jp-artist' title="+s.artist+">主播： " + s.artist + "</span>" : "") + "</a>", e += "</div></li>"
		},
